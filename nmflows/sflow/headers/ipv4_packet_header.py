...
class IPv4PacketHeader:
    ...
    def unpack(cls, data: bytes):
        ihl = 4 * (int.from_bytes(data[0:1], byteorder='big', signed=False) & 0xf)
        proto = int.from_bytes(data[9:10], 'big', signed=False) & 0xff
        src_addr = socket.inet_ntop(socket.AF_INET, data[12:16])
        dst_addr = socket.inet_ntop(socket.AF_INET, data[16:20])
        t_length = int.from_bytes(data[2:4], byteorder='big', signed=False) & 0xffff
